...
function viewWindow( str ){ 
	if( str === 'width' ){ 
		return document.documentElement.clientWidth || documet.body.clientWidth;
	}else if( str === 'height' ){ 
...
function aqiSort( arr , blrean ){ 
	...
	var b = str.split('|');
	b.pop(1);
	var index = Math.floor(b.length/2);
	var num = parseInt( b.splice( index , 1 )[0] );
	var left = [];
	var right = [];
	for( var i=0; i<b.length; i++ ){
		b[i] < num ?  left.push( parseInt( b[i] ) ) : right.push( parseInt( b[i] ) );
	};
